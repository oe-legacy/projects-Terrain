...
landscape and sky and rendering it to the screen. Now we turn to
screen space effects, which do not rely on geometry, but will instead
use the final color image and the depth buffer to create the
effects. \emph{Depth of field}, \emph{motion blur} and a screenwide
\emph{glow} are all effects that can easily be produced through post
processing and there are many more.
...
the \emph{separable convolution technique}.
...
shader abstraction, \class{IShaderResource}, that points to the
effect. It contains a pointer to an OpenEngine \class{FrameBuffer},
which the scene is rendered to. The postprocess node also holds a
\class{FrameBuffer} where the final image can be blitted into after
the effect has been applied. This is useful for some implementations
of motion blur, that relies on the previous image being availible.
...
kernel, as can be seen on \reffig{fig:blurKernels}. To increase
effectivness we tried squaring the error term and came up with the
kernel in \reffig{fig:blurKernels}, which while faster still provided
a pleasing result.
...
\begin{figure}
  ...
  \subfloat[The landscape is motionblurred as teh camera pans to the side.]{
